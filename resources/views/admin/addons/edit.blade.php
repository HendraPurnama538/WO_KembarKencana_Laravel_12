...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
            ...
            <h3 class="font-semibold text-slate-800">Edit Add-on: {{ $addon->name }}</h3>
        </div>

        <form method="POST" action="{{ route('admin.addons.update', $addon) }}" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="category" name="category"
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat }}" {{ old('category', $addon->category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="unit" name="unit"
                        ...
                        @foreach($units as $u)
                            <option value="{{ $u }}" {{ old('unit', $addon->unit) === $u ? 'selected' : '' }}>{{ $u }}</option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          ...
                          class="w-full rounded-xl border-slate-300 text-sm focus:border-violet-400 focus:ring-violet-400">{{ old('description', $addon->description) }}</textarea>
            </div>
                ...
                <div class="flex gap-4">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="active"
                               {{ old('status', $addon->status) === 'active' ? 'checked' : '' }} class="accent-violet-600">
                    ...
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="inactive"
                               {{ old('status', $addon->status) === 'inactive' ? 'checked' : '' }} class="accent-violet-600">
                        <span class="text-sm text-slate-700 inline-flex items-center gap-1">
