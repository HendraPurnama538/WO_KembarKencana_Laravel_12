...
@section('title', 'Edit Paket Pernikahan')
@section('subtitle', 'Perbarui detail paket pernikahan')

@section('content')
<div class="max-w-3xl">
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-800">Edit Paket: {{ $package->name }}</h3>
        </div>

        <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="p-6 space-y-5" id="pkg-form">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select id="batch" name="batch"
                        ...
                        @foreach([1,2,3,4] as $b)
                            <option value="{{ $b }}" {{ old('batch', $package->batch) == $b ? 'selected' : '' }}>Batch {{ $b }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full rounded-xl border-slate-300 text-sm focus:border-violet-400 focus:ring-violet-400 @error('description') border-red-400 @enderror">{{ old('description', $package->description) }}</textarea>
            ...
            <div>
                ...
                <p id="no-category-hint" class="text-xs text-slate-400 italic mt-2" style="display:none;">
                ...
                <div class="flex gap-4">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="active"
                               {{ old('status', $package->status ?? 'active') === 'active' ? 'checked' : '' }}
...
{{-- ── Embed existing facilities_json for JS to load ── --}}
<script id="existing-facilities-json" type="application/json">
@php
    // Priority: old() input on validation error → existing package data
    $oldCats  = old('fac_category');
    if ($oldCats !== null) {
        // Rebuild from old() input
        $oldItems = old('fac_items', []);
        $existingJson = [];
        foreach ($oldCats as $idx => $cat) {
            if (trim($cat) === '') continue;
            $items = array_values(array_filter(array_map('trim', $oldItems[$idx] ?? []), fn($v) => $v !== ''));
            $existingJson[] = ['category' => trim($cat), 'items' => $items];
        }
    } else {
        // Load from saved facilities_json
        $existingJson = $package->facilities_json ?? [];
    }
    echo json_encode($existingJson, JSON_UNESCAPED_UNICODE);
@endphp
</script>
...
(function () {
    ...
    window.addCategoryWithName = function(name) {
        ...
        updateHint();
        block.querySelector('.btn-add-item').click();
    };
    ...
    // ── Load existing data ──────────────────────────────────────────────────
    const dataEl = document.getElementById('existing-facilities-json');
    if (dataEl) {
        try {
        ...
        } catch (e) { console.warn('facilities_json parse error', e); }
    }
    updateHint();
