...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input id="price" name="price" type="number" value="{{ old('price', $package->price) }}"
                           min="0" step="1000"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full rounded-xl border-slate-300 text-sm focus:border-violet-400 focus:ring-violet-400
                                 @error('description') border-red-400 @enderror">{{ old('description', $package->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="facilities" name="facilities" rows="5"
                          class="w-full rounded-xl border-slate-300 text-sm focus:border-violet-400 focus:ring-violet-400
                                 @error('facilities') border-red-400 @enderror">{{ old('facilities', $package->facilities) }}</textarea>
                @error('facilities')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
