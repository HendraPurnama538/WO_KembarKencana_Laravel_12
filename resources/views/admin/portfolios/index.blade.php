...
@section('subtitle', 'Daftar semua portofolio pernikahan')
...
<div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
        <div>
            <h3 class="font-semibold text-slate-800">Daftar Portofolio</h3>
            <p class="text-xs text-slate-400 mt-0.5">Total: {{ $portfolios->total() }} portofolio</p>
        </div>
        <a href="{{ route('admin.portfolios.create') }}"
           class="inline-flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white text-sm font-medium px-4 py-2 rounded-xl transition-colors">
    ...
    @if($portfolios->isEmpty())
        <div class="py-16 text-center text-slate-400">
            ...
            <p class="text-sm">Belum ada portofolio. Tambahkan sekarang!</p>
        </div>
    @else
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wide">
                        ...
                        <th class="px-5 py-3 text-left font-semibold">Gambar</th>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/70 transition-colors">
                        ...
                        <td class="px-5 py-3">
                            <p class="font-medium text-slate-800">{{ $portfolio->title }}</p>
                        </td>
                        <td class="px-5 py-3 max-w-xs">
                            <p class="text-slate-500 truncate">{{ Str::limit($portfolio->description, 60) }}</p>
                        </td>
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.portfolios.edit', $portfolio) }}"
                                   class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium bg-sky-50 text-sky-600 hover:bg-sky-100 rounded-lg transition-colors">
                                    Edit
                                </a>

                                {{-- Tombol hapus — trigger via JS --}}
                                <button type="button"
                                    onclick="hapusData('{{ route('admin.portfolios.destroy', $portfolio) }}', 'Hapus portofolio &ldquo;{{ addslashes($portfolio->title) }}&rdquo;?')"
                                    class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors">
                                    Hapus
                                </button>
                            </div>
                        </td>
