...
@section('subtitle', 'Perbarui data vendor atau partner')
...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
            ...
            <h3 class="font-semibold text-slate-800">Edit Vendor — {{ $vendor->name }}</h3>
        </div>

        <form method="POST" action="{{ route('admin.vendors.update', $vendor) }}" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name', $vendor->name) }}"
            ...
            <div>
                ...
                <input id="handle" name="handle" type="text" value="{{ old('handle', $vendor->handle) }}"
            ...
            <div>
                ...
                <input id="instagram_url" name="instagram_url" type="url" value="{{ old('instagram_url', $vendor->instagram_url) }}"
            ...
            <div>
                ...
                <div id="current-image-section" class="mb-3 flex items-start gap-3">
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        {{-- Toggle Button for Remove --}}
                        <label class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors w-fit cursor-pointer group">
                            <input type="checkbox" name="remove_image" value="1" class="w-3.5 h-3.5 rounded border-red-300 text-red-600 focus:ring-red-500 cursor-pointer hidden" onchange="toggleRemoveImage(this)">
                ...
                <div class="flex items-start gap-4" id="upload-new-section">
                    {{-- Preview gambar baru --}}
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          ...
                          class="w-full rounded-xl border-slate-300 text-sm focus:border-violet-400 focus:ring-violet-400">{{ old('description', $vendor->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="active" {{ old('status', $vendor->status) === 'active' ? 'checked' : '' }} class="accent-violet-600">
                    ...
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="inactive" {{ old('status', $vendor->status) === 'inactive' ? 'checked' : '' }} class="accent-violet-600">
                        <span class="text-sm text-slate-700 inline-flex items-center gap-1">
...
function toggleRemoveImage(checkbox) {
    ...
    const label = checkbox.nextElementSibling.nextElementSibling; // the <span> element
    if (checkbox.checked) {
        img.classList.add('opacity-50', 'grayscale');
        label.innerText = 'Batal Hapus';
    } else {
